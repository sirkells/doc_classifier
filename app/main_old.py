...
def tokenize(text):
    ...
    try:
        ...
        tokens = word_tokenize(text)
        stemmed = [
            stemmer_germ.stem(word) for word in tokens if word not in stopwords_sl
        ]
...
def tokenize_ul(text):
    ...
    try:
        ...
        tokens = word_tokenize(text)
        stemmed = [
            stemmer_germ.stem(word) for word in tokens if word not in stopwords_ul
        ]
        stemmed = [word for word in stemmed if len(word) > 1]
...
def predict_bereich(text, lda_model):
    ...
    topic_distr_array = np.array(
        [topic[1] for topic in lda_model.get_document_topics(bow=text_bow)]
    )
...
def find_top_similar_docs(doc_distr_query, corpus_distr, n=10):
    ...
    similar_docs_index_array = sim_score.argsort()[
        :n
    ]  # argsort sorts from lower to higher
...
def recommend(text):
    ...
    new_doc_distribution = np.array(
        [tup[1] for tup in lda_model.get_document_topics(bow=text_bow)]
    )
    corpus_topic_dist = np.array(
        [[topic[1] for topic in docs] for docs in all_topic_distr_list]
    )
    similar_docs_index = find_top_similar_docs(
        new_doc_distribution, corpus_topic_dist, 10
    )
...
def home():
    ...
    group = [
        ...
        "SW_Dev/DevOps",
        "Sys_Admin/Support",
        "SW_Dev/Mobile/support",
        "Data/Ops",
        "IT_Process_Mgr/Consultant",
        "MS_DEV/Admin",
        "Business_Analyst/Consulting",
    ]
    ...
    if labels_perc[index_highest] < 20:
        ...
        return render_template("home.html", query=query, output={}, labels={}, group={})
    else:
